...
const Persons = (props) => {
  ...
  return(
    <div>
      {persons.map(person => <Person key={person.name} id={person.id} name={person.name} phone={person.number} removePerson={removePerson}/>)}
    </div>
  )
}
    ...
    if(p){
      personService
        ...
        .catch(error => {          
          ...
          //alert(`${new_person} was already deleted from server`)
      ...
      personService
        ...
        .then(returnedPerson => {
          ...
          console.log(`Se agrego la persona ${returnedPerson}`);
        })            
    }
  }
  ...
  const removePerson = (id, nombre) => {
    ...
    if(election){
      personService.remove(id)
        ...
        .catch(error => {
          //alert(`No se ha podido eliminar a ${nombre}`)
